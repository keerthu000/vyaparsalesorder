        ...
        if 'staff_id' in request.session:
          if request.session.has_key('staff_id'):
            ...
            if data.company.End_date == current_day:
              ...
              elif Distributor_Notification.objects.filter(company_id = data.company.id ,status = 'New').exists():
    ...
    if request.method == 'POST':
        ...
        if LoanAccounts.objects.filter(account_number=account_number  ,company_id=cmp).exists():
            ...
            TransactionTable.objects.create(
                ...
                balance_amount=total_amount,company=cmp
            )
...
def edit_loan_page_function(request, eid):
    ...
    if request.method == 'POST':
        ...
        emipayment= TransactionTable.objects.filter(loan_account=eid, transaction_type=transaction_type) 

      
        total_payment_sum = sum(emipayment.values_list('payment', flat=True))

      
        new_total_amount = loan_account.loan_amount + loan_account.interest_rate

        for payment_entry in emipayment:
            payment_entry.balance_amount = new_total_amount - total_payment_sum
            payment_entry.save()

    
        loan_account.total_amount = new_total_amount
        loan_account.save()
...
def make_payment(request, eid):
    ...
    if request.method == 'POST':
        ...
        balance_amount = Decimal(amount) - Decimal(total_amount)
...
def additional_loan_function(request, eid):
    ...
    if request.method == 'POST':
        ...
        last_balance_amount= TransactionTable.objects.filter(loan_account=eid).last()
        amount= last_balance_amount.balance_amount
        balance_amount = Decimal(amount) + Decimal(total_loan)
    ...
    if request.method == 'POST':
      ...
      if Distributors_details.objects.filter(user = request.user).exists():
        ...
        if com.reg_action == 'self':      
          ...
          noti = Distributor_Notification(distributor_id=com.Distributors.id,company_id=com,user_Id = data,PaymentTerms_updation = data1,Title = "Change Payment Terms",Discription = com.user.first_name+''+ com.user.last_name+ " is change Payment Terms")
          noti.save()
